...
CREDITS_PER_USD = int(os.getenv("CREDITS_PER_USD", "10000"))
BILLING_MULTIPLIER = float(os.getenv("BILLING_MULTIPLIER", "1.0"))
MODEL_INPUT_PRICE_PER_MTOK = float(os.getenv("MODEL_INPUT_PRICE_PER_MTOK", "3.0"))
MODEL_OUTPUT_PRICE_PER_MTOK = float(os.getenv("MODEL_OUTPUT_PRICE_PER_MTOK", "15.0"))
CACHE_WRITE_PRICE_PER_MTOK = float(os.getenv("CACHE_WRITE_PRICE_PER_MTOK", "0.0"))
CACHE_READ_PRICE_PER_MTOK = float(os.getenv("CACHE_READ_PRICE_PER_MTOK", "0.0"))
MIN_REQUEST_CREDITS = int(os.getenv("MIN_REQUEST_CREDITS", "1"))
...
def calculate_request_cost_usd(response) -> tuple[float, dict]:
    ...
    input_cost = (input_tokens / 1_000_000) * MODEL_INPUT_PRICE_PER_MTOK
    output_cost = (output_tokens / 1_000_000) * MODEL_OUTPUT_PRICE_PER_MTOK
    cache_write_cost = (cache_creation_input_tokens / 1_000_000) * CACHE_WRITE_PRICE_PER_MTOK
    cache_read_cost = (cache_read_input_tokens / 1_000_000) * CACHE_READ_PRICE_PER_MTOK

    total_cost = (input_cost + output_cost + cache_write_cost + cache_read_cost) * BILLING_MULTIPLIER
    breakdown = {
...
def convert_usd_to_credits(cost_usd: float) -> int:
    credits = int(round(cost_usd * CREDITS_PER_USD))
    return max(credits, MIN_REQUEST_CREDITS)
...
def chat(data: ChatRequest):
    ...
    cost_usd, usage_breakdown = calculate_request_cost_usd(resp)
    credits_needed = convert_usd_to_credits(cost_usd)

    if row["credits"] < credits_needed:
        conn.close()
        raise HTTPException(
            status_code=402,
            detail=f"额度不足：本次真实成本约需 {credits_needed} 点，请联系客服充值。当前剩余 {row['credits']} 点。",
        )
